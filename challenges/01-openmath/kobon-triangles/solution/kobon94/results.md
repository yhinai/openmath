...
- N=16: first UNSAT K=0, first SAT K=—; TIMEOUT at K=[2, 3, 4, 5, 6]
...
- N=29: first UNSAT K=—, first SAT K=—; TIMEOUT at K=[0, 1, 2, 3, 4, 5, 6]
...
**Portfolio 1 is fully spent with no verdict**: all 17 seeds are logged TIMEOUT at
2 h 32 m (9211 s), and none of them hit the 6 h cap — every job shows
`raising signal 15 (SIGTERM)` at the same second (23:14:01) after a peak RSS of only
~150 MB per job, i.e. an external SIGTERM, not a timeout and not OOM. `day_deep.py`
skips rows already in `deep_results.txt`, so it will never retry that band.

A **portfolio 2** (`day_deep2.py`, seeds 10–17 on K=6 and 3–5 on K=5,7,8, separate
append-only `deep_results2.txt`, same 6 h cap) was launched at 23:19 to put the decisive
band under attack again with genuinely independent runs; it is registered in
`launch_all.py`, so the `*/5` watchdog keeps it alive too. Launch history for the deep
attack (4 earlier launches, no verdict recorded from any):
...
4. 20:40:30 — 17 seeds, all externally SIGTERM'd at 23:14:01 (2 h 32 m), no verdict;
   portfolio 1 spent.
...
Verified by drat-trim: 11:0, 11:1, 11:2, 12:0, 12:1, 12:2, 13:0, 13:1, 16:0, 16:1, 18:0, 18:1, 19:0 — 13 rows, all `s VERIFIED`
